    ...
    <>
      ...
      >
        ...
        {userPartitionInfo?.selectablePartitions.map((partition, index) => (
          <option key={partition.id} value={index}>
            {partition.name}
          </option>
        ))}
      </Form.Control>
        ...
        <Form.Group controlId="select-groups-checkboxes">
          ...
          >
            {userPartitionInfo.selectablePartitions[selectedPartitionIndex].groups.map((group) => (
              <Form.Group key={group.id} className="pgn__form-checkbox">
                <Field
                  as={Form.Control}
                ...
                />
                <div>
                  <Form.Label className={classNames({ 'text-danger': checkIsDeletedGroup(group) })} isInline>
                    {group.name}
                  </Form.Label>
                  {
...
export const UnitTab = ({
  ...
  const { isVisibleToStaffOnly, selectedPartitionIndex, selectedGroups, discussionEnabled } = values;

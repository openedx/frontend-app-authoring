...
import { CLIPBOARD_STATUS, STRUCTURAL_XBLOCK_TYPES, STUDIO_CLIPBOARD_CHANNEL } from '@src/constants';
...
const useClipboard = (canEdit: boolean = true) => {
  ...
  const showPasteXBlock =
    isPasteable && clipboardData?.content && !STRUCTURAL_XBLOCK_TYPES.includes(clipboardData.content?.blockType);

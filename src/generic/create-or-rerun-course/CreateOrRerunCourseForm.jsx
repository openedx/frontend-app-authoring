...
import { Form, Button, Dropdown, ActionRow, StatefulButton, TransitionReplace } from '@openedx/paragon';
...
const CreateOrRerunCourseForm = ({ title, isCreateNewCourse, initialValues, onClickCancel }) => {
  ...
  const newCourseFields = [
    {
      ...
      helpText: intl.formatMessage(
        isCreateNewCourse ? messages.courseDisplayNameCreateHelpText : messages.courseDisplayNameRerunHelpText,
      ),
      name: 'displayName',
      ...
      helpText: isCreateNewCourse
        ...
        : intl.formatMessage(messages.courseOrgRerunHelpText, {
            strong: (
              <>
                <br />
                <strong>{intl.formatMessage(messages.courseNoteNoSpaceAllowedStrong)}</strong>
              </>
            ),
          }),
      name: 'org',
    ...
    },
    {
      ...
      helpText: isCreateNewCourse
        ? intl.formatMessage(messages.courseNumberCreateHelpText, {
            strong: <strong>{intl.formatMessage(messages.courseNotePartCourseURLRequireStrong)}</strong>,
          })
      ...
      <Form>
        {newCourseFields.map((field) => (
          ...
          >
            ...
            {hasErrorField(field.name) && (
              <Form.Control.Feedback className="feedback-error" type="invalid" hasIcon={false}>
                {errors[field.name]}
        ...
        <ActionRow className="justify-content-start">
          ...
          <StatefulButton
            ...
            state={
              savingStatus === RequestStatus.PENDING ? STATEFUL_BUTTON_STATES.pending : STATEFUL_BUTTON_STATES.default
            }
            {...createButtonState}
          />
        </ActionRow>

...
import { fireEvent, initializeMocks, render as baseRender, waitFor } from '../testUtils';
...
describe('<TaxonomyListPage />', () => {
  ...
  it('should show all "All taxonomies", "Unassigned" and org names in taxonomy org filter', async () => {
    ...
    const { getByTestId, getByText, getByRole, getAllByText, queryByText } = render(<TaxonomyListPage />);
    // Wait until data has been loaded and rendered:
    await waitFor(() => {
      expect(queryByText('Loading')).toEqual(null);
    });
  ...
  it('should fetch taxonomies with correct params for org filters', async () => {
    ...
    const { getByRole, getByText, queryByText, findByRole } = render(<TaxonomyListPage />);

...
const TaxonomyDetailSideCard = ({ taxonomy }) => {
  ...
  return (
    <Card>
      ...
      <Card.Section title={intl.formatMessage(messages.taxonomyDetailsName)}>{taxonomy.name}</Card.Section>
      <Card.Divider className="ml-3 mr-3" />
      <Card.Section title={intl.formatMessage(messages.taxonomyDetailsDescription)}>
        {taxonomy.description}
      </Card.Section>
      <Card.Section title={intl.formatMessage(messages.taxonomyDetailsExportID)}>{taxonomy.exportId}</Card.Section>
    </Card>
  );
};

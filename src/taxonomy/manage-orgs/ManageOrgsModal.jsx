...
import { useToggle, ActionRow, AlertModal, Button, Chip, Container, Form, ModalDialog, Stack } from '@openedx/paragon';
import { Close, Warning } from '@openedx/paragon/icons';
...
const ConfirmModal = ({ isOpen, onClose, confirm, taxonomyName }) => {
  const intl = useIntl();
  return (
    <AlertModal
    ...
    >
      <p>{intl.formatMessage(messages.confirmUnassignText, { taxonomyName })}</p>
    </AlertModal>
  );
};
...
const ManageOrgsModal = ({ taxonomyId, isOpen, onClose }) => {
  const intl = useIntl();
  ...
  const { data: organizationListData } = useOrganizationListData();
  ...
  useEffect(() => {
    if (selectedOrgs) {
      ...
      const inputRef = /** @type {null|HTMLInputElement} */ (
        document.querySelector('.manage-orgs .pgn__form-group input')
      );
      if (inputRef) {
  ...
  return (
    <Container onClick={(e) => e.stopPropagation() /* This prevents calling onClick handler from the parent */}>
      ...
      >
        ...
        <ModalDialog.Header>
          <ModalDialog.Title>{intl.formatMessage(messages.headerTitle)}</ModalDialog.Title>
        </ModalDialog.Header>
        ...
        <ModalDialog.Body>
          ...
          <Form.Group>
            <Form.Label>{intl.formatMessage(messages.addOrganizations)}</Form.Label>
            <Form.Autosuggest
            ...
            >
              {organizationListData
                ? organizationListData
                    ...
                    .map((org) => <Form.AutosuggestOption key={org}>{org}</Form.AutosuggestOption>)
                : []}
            </Form.Autosuggest>
          </Form.Group>

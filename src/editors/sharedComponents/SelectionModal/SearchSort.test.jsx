...
import { filterKeys, filterMessages, sortKeys, sortMessages } from '../../containers/VideoGallery/utils';
...
describe('SearchSort component', () => {
  ...
  test('adds a sort option for each sortKey', () => {
    ...
    Object.values(sortMessages).forEach(({ defaultMessage }) => {
      expect(getByRole('link', { name: `By ${defaultMessage}` })).toBeInTheDocument();
    });
  });
  test('adds a sort option for each sortKey', () => {
    const { getByRole } = getComponent();
  ...
  test('adds a filter option for each filter key', () => {
    ...
    Object.keys(filterMessages).forEach((key) => {
      expect(getByTestId(`dropdown-filter-${key}`)).toBeInTheDocument();
    });
  });
  test('searchbox should show clear message button when not empty', () => {
    const { queryByRole } = getComponent({ searchString: 'some string' });
    expect(queryByRole('button', { name: messages.clearSearch.defaultMessage })).toBeInTheDocument();
  });
});

...
export const networkRequest =
  ({ requestKey, promise, onSuccess, onFailure }) =>
  (dispatch) => {
...
export const batchUploadAssets =
  ({ assets, content, ...rest }) =>
  (dispatch) => {
    const promises = assets.reduce(
      (promiseChain, asset) =>
        promiseChain.then(
          (parsedContent) =>
            new Promise((resolve) => {
...
export const checkTranscriptsForImport =
  ({ videoId, youTubeId, ...rest }) =>
  (dispatch, getState) => {
    dispatch(
      module.networkRequest({
...
export const uploadTranscript =
  ({ transcript, videoId, language, ...rest }) =>
  (dispatch, getState) => {
    const state = getState();
    const isLibrary = selectors.app.isLibrary(state);
    if (isLibrary) {
...
export const updateTranscriptLanguage =
  ({ file, languageBeforeChange, newLanguageCode, videoId, ...rest }) =>
  (dispatch, getState) => {
    const state = getState();
    const isLibrary = selectors.app.isLibrary(state);
    if (isLibrary) {
...
export const fetchVideoFeatures =
  ({ ...rest }) =>
  (dispatch, getState) => {
    dispatch(
      module.networkRequest({

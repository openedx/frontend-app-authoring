...
export const loadVideoData = (selectedVideoId, selectedVideoUrl) => (dispatch, getState) => {
  ...
  const { videoId, videoUrl, fallbackVideos } = module.determineVideoSources({
...
export const determineVideoSources = ({ edxVideoId, youtubeId, html5Sources }) => {
  ...
  if (level === 'block') {
    ...
    metadataArr.forEach((arr) => {
      ...
      if (parsedStr.includes('license')) {
        license = parsedStr
          .substring(parsedStr.indexOf('"value"'), parsedStr.indexOf(', "type"'))
          .replace(/"value": |"/g, '');
      }
    });
  }
...
export const updateTranscriptLanguage =
  ({ newLanguageCode, languageBeforeChange }) =>
  (dispatch, getState) => {
    const state = getState();
    const {
    ...
    dispatch(
      requests.getTranscriptFile({
        videoId,
        language: languageBeforeChange,
        onSuccess: (response) => {
          dispatch(
            requests.updateTranscriptLanguage({
...
export const uploadVideo =
  ({ supportedFiles, setLoadSpinner, postUploadRedirect }) =>
  (dispatch) => {
    const data = { files: [] };
    setLoadSpinner(true);

...
describe('header utils', () => {
  describe('getContentMenuItems', () => {
    it('when video upload page enabled should include Video Uploads option', () => {
      ...
      const actualItems = renderHook(() => useContentMenuItems('course-123'), { wrapper: createWrapper() }).result
        .current;
  ...
  describe('getSettingsMenuitems', () => {
    ...
    it('when certificate page enabled should include certificates option', () => {
      ...
      const actualItems = renderHook(() => useSettingMenuItems('course-123'), { wrapper: createWrapper() }).result
        .current;
    ...
    it('when user has access to advanced settings should include advanced settings option', () => {
      const actualItemsTitle = renderHook(() => useSettingMenuItems('course-123'), {
        wrapper: createWrapper(),
      }).result.current.map((item) => item.title);
  ...
  describe('getToolsMenuItems', () => {
    it('when tags enabled should include export tags option', () => {
      ...
      const actualItemsTitle = renderHook(() => useToolsMenuItems('course-123'), {
        wrapper: createWrapper(),
      }).result.current.map((item) => item.title);
      expect(actualItemsTitle).toEqual(['Import', 'Export Course', 'Export Tags', 'Checklists']);
    });
    it('when tags disabled should not include export tags option', () => {
      ...
      expect(actualItemsTitle).toEqual(['Import', 'Export Course', 'Checklists']);
    });
    ...
    it('when course optimizer enabled should include optimizer option', () => {
      ...
      const optimizerItem = renderHook(() => useToolsMenuItems('course-123'), {
        wrapper: createWrapper(),
      }).result.current.find((item) => item.href === '/course/course-123/optimizer');
  ...
  describe('useLibrarySettingsMenuItems', () => {
    it('should contain team access url', () => {
      const items = renderHook(() => useLibrarySettingsMenuItems('library-123', false), { wrapper: createWrapper() })
        .result.current;
    ...
    it('should contain admin console url if set and readOnly is true', () => {
      ...
      const items = renderHook(() => useLibrarySettingsMenuItems('library-123', true), { wrapper: createWrapper() })
        .result.current;
      expect(items).toContainEqual({
        title: 'Library Team',
        href: 'http://admin-console.com/authz/libraries/library-123',
  ...
  describe('useLibraryToolsMenuItems', () => {
    it('should contain backup and import url', () => {
      const items = renderHook(() => useLibraryToolsMenuItems('course-123'), { wrapper: createWrapper() }).result
        .current;
      expect(items).toContainEqual({

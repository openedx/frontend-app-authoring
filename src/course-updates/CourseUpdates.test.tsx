...
import { initializeMocks, render, waitFor, fireEvent, screen } from '@src/testUtils';

import { getCourseUpdatesApiUrl, getCourseHandoutApiUrl, updateCourseUpdatesApiUrl } from './data/api';
import {
...
describe('<CourseUpdates />', () => {
  describe('Successful API responses', () => {
    beforeEach(() => {
      ...
      axiosMock.onGet(getCourseUpdatesApiUrl(courseId)).reply(200, courseUpdatesMock);
    ...
    it('should edit course update', async () => {
      ...
      axiosMock.onPut(updateCourseUpdatesApiUrl(courseId, courseUpdatesMock[0].id)).reply(200, data);
  ...
  describe('page load failure API responses', () => {
    ...
    it('Course updates fetch should show updates loading error', async () => {
      axiosMock.onGet(getCourseUpdatesApiUrl(courseId)).reply(404);
      axiosMock.onGet(getCourseHandoutApiUrl(courseId)).reply(200, courseHandoutsMock);
    ...
    it('Course handouts fetch should show handouts loading error', async () => {
      axiosMock.onGet(getCourseUpdatesApiUrl(courseId)).reply(200, courseUpdatesMock);
      axiosMock.onGet(getCourseHandoutApiUrl(courseId)).reply(404);
    ...
    it('displays an alert and sets status to DENIED when API responds with 403', async () => {
      axiosMock.onGet(getCourseUpdatesApiUrl(courseId)).reply(403, courseUpdatesMock);
      axiosMock.onGet(getCourseHandoutApiUrl(courseId)).reply(403);

      render(<RootWrapper />);

      await waitFor(() => {
        const { loadingStatuses } = store.getState().courseUpdates;
        Object.values(loadingStatuses).some((status) => expect(status).toEqual(RequestStatus.DENIED));
      });
  ...
  describe('saving failure API responses', () => {
    ...
    it('creating new update should show saving error alert', async () => {
      ...
      axiosMock.onPost(getCourseUpdatesApiUrl(courseId), data).reply(404);
    ...
    it('editing course update should show saving error alert', async () => {
      ...
      axiosMock.onPut(updateCourseUpdatesApiUrl(courseId, courseUpdatesMock[0].id)).reply(404);
    ...
    it('deleting course update should show delete saving error alert', async () => {
      ...
      axiosMock.onDelete(updateCourseUpdatesApiUrl(courseId, courseUpdatesMock[0].id)).reply(404);
    ...
    it('editing course handouts should show saving error alert', async () => {
      ...
      axiosMock.onPut(getCourseHandoutApiUrl(courseId)).reply(404);

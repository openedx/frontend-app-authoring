...
/**
 * Mock for `getCollection()`
 *
 * This mock returns different data/responses depending on the ID of the library & collection
 * that you request.
 */
export async function mockCollection(libraryId: string, collectionId: string): Promise<api.Collection> {
  // This mock has many different behaviors, depending on the collection ID:
  switch (collectionId) {
    case mockCollection.collectionIdThatNeverLoads:
      // Return a promise that never resolves, to simulate never loading:
      return new Promise<any>(() => {});
    case mockCollection.collection404:
      throw createAxiosError({ code: 400, message: 'Not found.', path: api.getLibraryCollectionApiUrl(libraryId, collectionId) });
    case mockCollection.collection500:
      throw createAxiosError({ code: 500, message: 'Internal Error.', path: api.getLibraryCollectionApiUrl(libraryId, collectionId) });
    case mockCollection.collectionId:
      return mockCollection.collectionData;
    default:
      throw new Error(`mockCollection: unknown collection ID "${collectionId}"`);
  }
}
mockCollection.libraryId = 'lib:Axim:TEST';
mockCollection.collectionId = 'my-first-collection';
mockCollection.collectionData = {
  // This is captured from a real API response:
  id: 1,
...
mockCollection.collectionIdThatNeverLoads = 'infiniteLoading-collection';
mockCollection.collection404 = 'collection-error404';
mockCollection.collection500 = 'collection-error500';
mockCollection.applyMock = () => jest.spyOn(api, 'getCollection').mockImplementation(mockCollection);

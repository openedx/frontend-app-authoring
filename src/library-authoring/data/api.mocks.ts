 ...
 */
export async function mockGetContainerMetadata(libraryId: string, containerId: string): Promise<api.Collection> {
  switch (collectionId) {
    case mockGetCollectionMetadata.collectionIdError:
      throw createAxiosError({
        code: 404,
        message: 'Not found.',
        path: api.getLibraryContainerApiUrl(libraryId, containerId),
      });
    case mockGetContainerMetadata.containerIdLoading:
      return new Promise(() => { });
    default:
      return Promise.resolve(mockGetContainerMetadata.collectionData);
  }
}

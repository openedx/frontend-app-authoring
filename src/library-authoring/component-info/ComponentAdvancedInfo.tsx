...
import { Alert, Button, Collapsible, OverlayTrigger, Tooltip } from '@openedx/paragon';
...
const ComponentAdvancedInfoInner: React.FC<Record<never, never>> = () => {
  ...
  const { data: olx, isLoading: isOLXLoading } = useXBlockOLX(usageKey, showOnlyPublished ? 'published' : 'draft');
  ...
  return (
    <>
      ...
      {(() => {
        ...
        if (!olx) {
          return <FormattedMessage {...messages.advancedDetailsOLXError} />;
        }
        return (
          <div className="mb-4">
            {olxUpdater.error ? (
            ...
            <CodeEditor key={usageKey} readOnly={!isEditingOLX} editorRef={editorRef}>
              {olx}
            </CodeEditor>
            {isEditingOLX ? (
              <>
...
export const ComponentAdvancedInfo: React.FC<Record<never, never>> = () => {
  const intl = useIntl();
  return (
    <Collapsible styling="basic" title={intl.formatMessage(messages.advancedDetailsTitle)}>

...
import { Button, Dropzone } from '@openedx/paragon';
...
export const ComponentAdvancedAssets: React.FC<Record<never, never>> = () => {
  ...
  const handleProcessUpload = React.useCallback(
    async ({ fileData, requestConfig, handleError }: { fileData: FormData; requestConfig: any; handleError: any }) => {
  ...
  return (
    <>
      ...
      {assets !== undefined && !readOnly ? ( // Wait until assets have loaded before displaying add button:
        <Dropzone style={{ height: '200px' }} onProcessUpload={handleProcessUpload} onUploadProgress={() => {}} />
      ) : null}

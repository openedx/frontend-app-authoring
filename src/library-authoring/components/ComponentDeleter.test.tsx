...
import { mockDeleteLibraryBlock, mockLibraryBlockMetadata } from '../data/api.mocks';
...
describe('<ComponentDeleter />', () => {
  ...
  it('is invisible when isConfirmingDelete is false', async () => {
    ...
    render(<ComponentDeleter usageKey={usageKey} isConfirmingDelete={false} cancelDelete={mockCancel} />);
  ...
  it('should shows a confirmation prompt the card with title and description', async () => {
    const mockCancel = jest.fn();
    render(<ComponentDeleter usageKey={usageKey} isConfirmingDelete cancelDelete={mockCancel} />);

...
describe('<ContainerInfoHeader />', () => {
  ...
  it('should update container title', async () => {
    ...
    const url = api.getLibraryContainerApiUrl(mockLibraryId, containerId);
    ...
    userEvent.type(textBox, 'New Container Title{enter}');

    await waitFor(() => {
      expect(axiosMock.history.patch[0].url).toEqual(url);
      expect(axiosMock.history.patch[0].data).toEqual(JSON.stringify({ display_name: 'New Container Title' }));
    });
  ...
  it('should not update container title if title is the same', async () => {
    ...
    const url = api.getLibraryCollectionApiUrl(mockLibraryId, containerId);
    axiosMock.onPatch(url).reply(200);
  ...
  it('should close edit container title on press Escape', async () => {
    ...
    userEvent.clear(textBox);
    userEvent.type(textBox, 'New Container Title{esc}');
  ...
  it('should show error on edit container title', async () => {
    ...
    await waitFor(() => {
      expect(axiosMock.history.patch[0].url).toEqual(url);
      expect(axiosMock.history.patch[0].data).toEqual(JSON.stringify({ title: 'New Container Title' }));
    });

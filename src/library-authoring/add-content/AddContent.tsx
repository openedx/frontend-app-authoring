...
import { Stack, Button, useToggle } from '@openedx/paragon';
import { useIntl } from '@edx/frontend-platform/i18n';
import { getConfig } from '@edx/frontend-platform';
import { AutoAwesome, KeyboardBackspace } from '@openedx/paragon/icons';
...
const AddContentButton = ({ contentType, onCreateContent }: AddContentButtonProps) => {
  const { name, disabled = false, icon, blockType } = contentType;
  return (
    <Button
...
const AddContentView = ({
  ...
  const componentButtons = contentTypes
  ...
  const visibleButtons = useMemo(() => {
    if (insideCollection) {
      ...
      return [existingContentButton, sectionButton, subsectionButton, unitButton, separator, ...componentButtons];
    }
    if (insideUnit) {
      // Only show existing content button + component buttons
      return [existingContentButton, separator, ...componentButtons];
    }
    if (insideSection) {
      // Only allow adding subsections
      return [existingContentButton, subsectionButton, pasteButton];
    }
    if (insideSubsection) {
      // Only allow adding units
      return [existingContentButton, unitButton, pasteButton];
    }
    // except for existing content, show everything.
    return [collectionButton, sectionButton, subsectionButton, unitButton, separator, ...componentButtons];
...
const AddContent = () => {
  ...
  const { showPasteUnit, showPasteSubsection, showPasteXBlock, isPasteable, sharedClipboardData } =
  ...
  const isBasicBlock = (blockType: string) => contentTypes.some((content) => content.blockType === blockType);

  const advancedBlocks = useMemo(
    () =>
      blockTypesData ? Object.fromEntries(Object.entries(blockTypesData).filter(([key]) => !isBasicBlock(key))) : {},
    [blockTypesData],
  ) as Record<string, BlockTypeMetadata>;
  ...
  const onCreateBlock = (blockType: string) => {
    ...
    if (suportedEditorTypes.includes(blockType)) {
      ...
      createBlockMutation
        ...
        .catch((error) => {
          showToast(parseErrorMsg(intl, error, messages.errorCreateMessageWithDetail, messages.errorCreateMessage));
        });
    }
  };
  ...
  const onCreateContent = (blockType: string) => {
    if (blockType === 'paste') {
      ...
      [ContainerType.Unit, ContainerType.Subsection, ContainerType.Section].includes(blockType as ContainerType)
    ) {

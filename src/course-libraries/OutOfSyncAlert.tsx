...
export const OutOfSyncAlert: React.FC<OutOfSyncAlertProps> = ({
  ...
  const lastPublishedDate = data
    ?.map((lib) => new Date(lib.lastPublishedAt || 0).getTime())
  ...
  return (
    <AlertMessage
      ...
      actions={[<Button onClick={onReview}>{intl.formatMessage(messages.outOfSyncCountAlertReviewBtn)}</Button>]}
    />
  );
};

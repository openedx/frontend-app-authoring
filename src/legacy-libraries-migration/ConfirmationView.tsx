...
import { Alert, Card, Container, Icon, Stack } from '@openedx/paragon';
import { AccessTime, Folder, SubdirectoryArrowRight } from '@openedx/paragon/icons';
...
const ConfirmationCard = ({ legacyLib, destinationName }: ConfirmationCardProps) => (
...
export const ConfirmationView = ({ destination, legacyLibraries }: ConfirmationViewProps) => (
  <Container className="confirmation-view">
    ...
    {legacyLibraries.map((legacyLib) => (
      <ConfirmationCard key={legacyLib.libraryKey} legacyLib={legacyLib} destinationName={destination.title} />
    ))}
  </Container>
);

...
const PublishModal = () => {
  ...
  const { id, displayName, category } = currentPublishModalData?.value || {};
  const categoryName = COURSE_BLOCK_NAMES[category || '']?.name.toLowerCase();
  const childInfo =
    currentPublishModalData?.value && 'childInfo' in currentPublishModalData.value
  ...
  return (
    ...
    >
      <ModalDialog.Header className="publish-modal__header">
        <ModalDialog.Title>{intl.formatMessage(messages.title, { title: displayName })}</ModalDialog.Title>
      </ModalDialog.Header>
      <ModalDialog.Body>
        <p className="small">{intl.formatMessage(messages.description, { category: categoryName })}</p>
        {children
          ?.filter((child) => child.hasChanges)
          .map((child) => {
            ...
            return grandChildren?.length ? (
              <React.Fragment key={child.id}>
                ...
                {grandChildren.map((grandChild) => (
                  <div key={grandChild.id} className="small border border-light-400 p-2 publish-modal__subsection">
                    {grandChild.displayName}
                  </div>
                ))}
              </React.Fragment>
            ) : (
              <div key={child.id} className="small border border-light-400 p-2 publish-modal__subsection">
                {child.displayName}
              </div>
            );
          })}

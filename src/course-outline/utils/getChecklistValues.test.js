...
describe('getChecklistValues utility function', () => {
  ...
  it('returns only checklist items with filters ALL and SELF_PACED when isSelfPaced is true', () => {
    ...
    filteredChecklist.forEach((item) =>
      expect(item.pacingTypeFilter === CHECKLIST_FILTERS.ALL || item.pacingTypeFilter === CHECKLIST_FILTERS.SELF_PACED),
    );

    expect(filteredChecklist.filter((item) => item.pacingTypeFilter === CHECKLIST_FILTERS.ALL).length).toEqual(
      checklist.filter((item) => item.pacingTypeFilter === CHECKLIST_FILTERS.ALL).length,
    );
    expect(filteredChecklist.filter((item) => item.pacingTypeFilter === CHECKLIST_FILTERS.SELF_PACED).length).toEqual(
      checklist.filter((item) => item.pacingTypeFilter === CHECKLIST_FILTERS.SELF_PACED).length,
    );
  });
  ...
  it('returns only checklist items with filters ALL and INSTRUCTOR_PACED when isSelfPaced is false', () => {
    ...
    filteredChecklist.forEach((item) =>
      expect(
        item.pacingTypeFilter === CHECKLIST_FILTERS.ALL || item.pacingTypeFilter === CHECKLIST_FILTERS.INSTRUCTOR_PACED,
      ),
    );
    ...
    expect(
      filteredChecklist.filter((item) => item.pacingTypeFilter === CHECKLIST_FILTERS.INSTRUCTOR_PACED).length,

...
import { WarningFilled as WarningIcon } from '@openedx/paragon/icons';

import messages from './messages';

const GradingPolicyAlert = ({ graded, gradingType, courseGraders }) => {
  const intl = useIntl();

  let gradingPolicyMismatch = false;
  if (graded) {
    if (gradingType) {
      gradingPolicyMismatch = courseGraders.filter((cg) => cg.toLowerCase() === gradingType.toLowerCase()).length === 0;
    }
  }

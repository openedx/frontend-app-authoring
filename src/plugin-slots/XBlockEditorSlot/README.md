...
Supply an external editor for the `invideoquiz` block type. `InVideoQuizEditor`
comes from a separate npm package that declares `react`, `@openedx/paragon` and
`@edx/frontend-platform` as peer dependencies.

```jsx
import InVideoQuizEditor from '@openedx-plugins/xblock-invideoquiz-editor';

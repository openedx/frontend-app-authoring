...
describe('getXBlockType utility', () => {
  it('returns section for chapter category', () => {
    const result = getXBlockType(CATEGORIES_KEYS.chapter);
    expect(result).toBe(CATEGORIES_KEYS.section);
  });

  it('returns subsection for sequential category', () => {
    const result = getXBlockType(CATEGORIES_KEYS.sequential);
    expect(result).toBe(CATEGORIES_KEYS.subsection);
  });

  it('returns unit for vertical category', () => {
    const result = getXBlockType(CATEGORIES_KEYS.vertical);
    expect(result).toBe(CATEGORIES_KEYS.unit);
  });
...
describe('getBreadcrumbs utility', () => {
  it('returns correct breadcrumb labels for visited ancestors', () => {
    const visitedAncestors: IAncestor[] = [
      { category: 'chapter', display_name: 'Chapter 1' },
      { category: 'section', display_name: 'Section 1' },
    ];
  ...
  it('returns base category label when category is course', () => {
    const visitedAncestors: IAncestor[] = [
      { category: CATEGORIES_KEYS.course, display_name: 'Course Name' },
    ];
  ...
  it('returns empty string if display_name is missing', () => {
    const visitedAncestors: IAncestor[] = [
      { category: 'chapter', display_name: '' },
    ];

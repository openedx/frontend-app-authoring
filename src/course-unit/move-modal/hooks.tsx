...
import {
  CATEGORIES_KEYS, CATEGORIES_TEXT, CATEGORY_RELATION_MAP, MOVE_DIRECTIONS,
} from './constants';
import {
...
export const useMoveModal = ({
  ...
  const initialValues = useMemo<IState>(() => ({
    childrenInfo: { children: courseOutlineInfo.child_info?.children ?? [], category: CATEGORIES_KEYS.section },
    parentInfo: { parent: courseOutlineInfo, category: CATEGORIES_KEYS.course },
    isValidMove: false,
  ...
  const updateChildrenItemsData = useCallback(() => {
    setState((prevState: IState) => ({
      ...
      childrenInfo: {
        ...
        children: prevState.parentInfo.parent?.child_info?.children || [],
      },
    }));
  }, []);

  const getCategoryText = useCallback(() => (
    intl.formatMessage(CATEGORIES_TEXT[state.childrenInfo.category]) || ''
  ), [intl, state.childrenInfo.category]);

  const breadcrumbs = getBreadcrumbs(state.visitedAncestors, intl.formatMessage);

  const setDisplayedXBlocksCategories = useCallback(() => {
    setState((prevState) => {
      const childCategory = CATEGORIES_KEYS.component;
      ...
      if (prevState.parentInfo.category !== newParentCategory) {
        return {
          ...
          childrenInfo: {
            ...prevState.childrenInfo,
            category: CATEGORY_RELATION_MAP[newParentCategory] || childCategory,
          },
        };
      }
      return prevState;
  ...
  useEffect(() => {
    if (isOpenModal && loadingStatus === RequestStatus.SUCCESSFUL) {
      updateParentItemsData();
    }
  }, [loadingStatus, isOpenModal, updateParentItemsData]);

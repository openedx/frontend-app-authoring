...
import { Button, FullscreenModal, Stepper } from '@edx/paragon';

import { PagesAndResourcesContext } from '../PagesAndResourcesProvider';

import messages from './messages';
import LiveProvider from './LiveProvider';
import { fetchLiveProviders } from './data/thunks';
...
function LiveSettings({ courseId, intl }) {
  const dispatch = useDispatch();
  const { path: pagesAndResourcesPath } = useContext(PagesAndResourcesContext);
  const { status, hasValidationError } = useSelector(state => state.live);
  const courseDetail = useModel('courseDetails', courseId);

  useEffect(() => {
    dispatch(fetchLiveProviders(courseId));
  }, [courseId]);

  const livePath = `${pagesAndResourcesPath}/live`;

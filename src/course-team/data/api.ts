...
export async function getCourseTeam(courseId: string): Promise<CourseTeam> {
  const { data } = await getAuthenticatedHttpClient().get(getCourseTeamApiUrl(courseId));
...
export async function createTeamUser(courseId: string, email: string) {
  await getAuthenticatedHttpClient().post(updateCourseTeamUserApiUrl(courseId, email), { role: USER_ROLES.staff });
}
...
export async function changeRoleTeamUser(courseId: string, email: string, role: string) {
  await getAuthenticatedHttpClient().put(updateCourseTeamUserApiUrl(courseId, email), { role });
}
...
export async function deleteTeamUser(courseId: string, email: string) {
  await getAuthenticatedHttpClient().delete(updateCourseTeamUserApiUrl(courseId, email));
}

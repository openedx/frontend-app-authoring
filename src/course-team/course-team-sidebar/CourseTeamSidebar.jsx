...
const CourseTeamSideBar = ({ courseId, isOwnershipHint, isShowInitialSidebar }) => {
  ...
  return (
    ...
    >
      <HelpSidebar intl={intl} courseId={courseId} showOtherSettings={false}>
        <h4 className="help-sidebar-about-title">{intl.formatMessage(messages.sidebarTitle)}</h4>
        <p className="help-sidebar-about-descriptions">{intl.formatMessage(messages.sidebarAbout_1)}</p>
        <p className="help-sidebar-about-descriptions">{intl.formatMessage(messages.sidebarAbout_2)}</p>
        <p className="help-sidebar-about-descriptions">{intl.formatMessage(messages.sidebarAbout_3)}</p>
        {courseTeamSupportEmail && (
          <>
            <p className="help-sidebar-other-title">
              <sub>
                <Icon src={InfoOutline} className="d-inline-block mr-2" style={{ height: 21, width: 21 }} />
              </sub>
              {intl.formatMessage(messages.helpInfoSidebarTitle)}
            </p>
            <p className="help-sidebar-about-descriptions">
              {intl.formatMessage(messages.helpInfoDescription, {
                email_address: <MailtoLink to={courseTeamSupportEmail}>{courseTeamSupportEmail}</MailtoLink>,
              })}
            </p>
          </>
        )}
      ...
      {isOwnershipHint && (
        <>
          <hr />
          <HelpSidebar intl={intl} courseId={courseId} showOtherSettings={false}>
            <h4 className="help-sidebar-about-title">{intl.formatMessage(messages.ownershipTitle)}</h4>
            <p className="help-sidebar-about-descriptions">
              {intl.formatMessage(messages.ownershipDescription, {

...
export async function getCourseAdvancedSettings(courseId: string): Promise<Record<string, any>> {
  const { data } = await getAuthenticatedHttpClient().get(`${getCourseAdvancedSettingsApiUrl(courseId)}?fetch_all=0`);
  const keepValues = {};
...
export async function updateCourseAdvancedSettings(
  ...
  const { data } = await getAuthenticatedHttpClient().patch(
    `${getCourseAdvancedSettingsApiUrl(courseId)}`,
    convertObjectToSnakeCase(settings),
  );
  const keepValues = {};
  Object.keys(data).forEach((key) => {
    keepValues[camelCase(key)] = { value: data[key].value };

 ...
 * }}
 */
const useContentTagsCollapsibleHelper = (contentId, stagedContentTags, taxonomyAndTagsData) => {
  const {
    isEditMode,
    addStagedContentTag,
  ...
  const { id, contentTags } = taxonomyAndTagsData;
  ...
  React.useEffect(() => {
    globalTagsToRemove.forEach((tag) => {
      if (globalStagedContentTags[id] && globalStagedContentTags[id].some((t) => t.value === tag)) {
  ...
  const commitStagedTagsToGlobal = React.useCallback(() => {
    ...
    explicitStaged.forEach((tag) => {
      if (globalStagedRemovedContentTags[id] && globalStagedRemovedContentTags[id].includes(tag.value)) {
    ...
    if (newGlobalTags) {
      if (!globalTags[id]) {
        ...
        const stagedLineages = stagedContentTags
          .map((st) => decodeURIComponent(st.value).split(',').slice(0, -1))
          .flat();
  ...
  const tagChangeHandler = React.useCallback(
    (tagSelectableBoxValue, checked) => {
  ...
  const removeAppliedTagHandler = React.useCallback(
    (tagSelectableBoxValue) => {
      const tagLineage = tagSelectableBoxValue.split(',').map((t) => decodeURIComponent(t));

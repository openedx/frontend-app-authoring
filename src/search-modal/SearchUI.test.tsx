...
describe('<SearchUI />', () => {
  ...
  it('defaults to searching "This Course" if used in a course', async () => {
    ...
    expect(fetchMock).toHaveLastFetched((_url, req) => {
      ...
      return (
        requestedFilter?.[1] === 'type = "course_block"' &&
  ...
  describe('results', () => {
    ...
    test('click lib component result navigates to course-authoring/library', async () => {
      ...
      expect(window.open).toHaveBeenCalledWith('/library/lib:org1:libafter1', '_blank');
      window.open = open;

      // Clicking in the result should navigate to the result's URL:
      fireEvent.click(resultItem);
      expect(mockNavigate).toHaveBeenCalledWith('/library/lib:org1:libafter1');
    });
  });
  ...
  describe('filters', () => {
    ...
    it('can filter results by component/XBlock type', async () => {
      ...
      expect(fetchMock).toHaveLastFetched((_url, req) => {
        ...
        const requestedFilter = requestData?.queries[0].filter;
        return (
          JSON.stringify(requestedFilter) ===
          JSON.stringify([
            [
              'block_type = problem',
    ...
    it('should update search results when keywords are entered', async () => {
      ...
      await waitFor(() => {
        expect(fetchMock).toHaveLastFetched((_url, req) => {
          ...
          return requestData?.queries[0]?.q === 'test search' && requestData?.queries[1]?.q === 'test search';
        });
      });
      expect(getByText('6 results found')).toBeInTheDocument();
      expect(getByText(mockResultDisplayName)).toBeInTheDocument();

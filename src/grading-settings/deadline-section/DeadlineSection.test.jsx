...
describe('<DeadlineSection />', () => {
  ...
  it('checking deadline input value', async () => {
    const { getByTestId } = render(
      <RootWrapper gracePeriod={gracePeriodDefaultTime} setGradingData={setGradingData} />,
    );
    await waitFor(() => {
  ...
  it('checking deadline input value if grace Period has no hours', async () => {
    const { getByTestId } = render(
      <RootWrapper gracePeriod={{ hours: 0, minutes: 13 }} setGradingData={setGradingData} />,
    );
    await waitFor(() => {
      const inputElement = getByTestId('deadline-period-input');
  ...
  it('checking deadline input value if grace Period has no minutes', async () => {
    const { getByTestId } = render(
      <RootWrapper gracePeriod={{ hours: 13, minutes: 0 }} setGradingData={setGradingData} />,
    );
    await waitFor(() => {
      const inputElement = getByTestId('deadline-period-input');

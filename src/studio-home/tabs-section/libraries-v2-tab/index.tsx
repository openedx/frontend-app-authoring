...
import { Icon, Row, Pagination, Alert, Button, Form, Stack, useToggle } from '@openedx/paragon';
...
const LibrariesV2List: React.FC<Props> = ({ selectedLibraryId, handleSelect, showCreateLibrary = false }) => {
  const intl = useIntl();
  ...
  const { data, isPending, isError } = useContentLibraryV2List({ page: currentPage, ...filterParams });

  const handlePostCreateLibrary = useCallback(
  ...
  const handleOnChangeRadioSet = useCallback(
    (libraryId: string) => {
    ...
    <div className="courses-tab-container">
      <div className="d-flex flex-row justify-content-between my-4">
        <Stack direction="horizontal">
          {showCreateLibrary && (
            <Button variant="outline-primary" onClick={openCreateLibrary} iconBefore={Add} className="mr-3">

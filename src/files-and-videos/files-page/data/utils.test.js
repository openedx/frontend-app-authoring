...
describe('updateFileValues - dateAdded conversion', () => {
  it('converts dateAdded to a numeric epoch timestamp (getTime), not a string', () => {
    ...
    // The whole point of the fix: dateAdded should now be a plain number (milliseconds since epoch), not a Date.toString()-style string like
  ...
  it('produces a dateAdded value that matches the actual parsed date', () => {
    ...
    // 'at' is stripped by the function before parsing (mirrors the API's "MMM dd, yyyy at HH:mm UTC" format), so this is the expected

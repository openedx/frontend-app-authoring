import { sortFiles, SortableFile } from './utils';

describe('sortFiles', () => {
  const files: SortableFile[] = [
  ...
  it('flips tied files consistently along with the sort direction', () => {
    ...
    // Under the old code, reversing the descending array with .reverse()
    // would flip ties unpredictably run to run. Under my first attempted
    // fix, tied files were frozen in the same relative order regardless
    // of direction, which was also wrong -- file8/file9 wouldn't flip
    // when toggled, even though every other (non-tied) file did.
    //
    // Under the current code, the id-based tiebreaker is multiplied by
    // directionMultiplier just like the primary comparison, so tied files
    // flip consistently along with the sort direction, same as any other
    // pair of files. That means Oldest should always be an exact,
    // item-for-item reversal of Newest, ties included.

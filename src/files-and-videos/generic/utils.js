...
export const sortFiles = (files, sortType) => {
  ...
  const sortedFiles = [...files].sort((f1, f2) => {
    ...
    // primaryComparison === 0 means a tie on the chosen sort field,
    // like two files uploaded in the same minute. This is the case the old code mishandled.
    // Instead of leaving this to "whatever order sort's internal stability happens to produce" (which is what caused the
    // inconsistency), we explicitly break the tie using `id` as every file has a unique ID. This ensures that the relative order of tied files is always consistent, regardless of sort direction.

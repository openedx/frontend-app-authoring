...
import { ActionRow, AlertModal, Button, Collapsible, DataTableContext, Hyperlink, Truncate } from '@openedx/paragon';
      ...
      <Collapsible
        ...
        title={
          <h3 className="h5 m-n2">
            <Truncate.Deprecated lines={1}>{original.displayName}</Truncate.Deprecated>
          </h3>
        }
    ...
    <AlertModal
      ...
      footerNode={
        <ActionRow>
          ...
          <Button onClick={handleConfirmDeletion}>{intl.formatMessage(messages.deleteFileButtonLabel)}</Button>
        </ActionRow>
      }
    >
      ...
      {isDeletingCourseContent && (
        <div className="mt-3">
          {intl.formatMessage(messages.deleteConfirmationUsageMessage, {
            fileNumber: activeContentRows.length,
            fileType,
          })}
          <ul className="p-0">{deletedCourseContent}</ul>
        </div>
      )}
    </AlertModal>
  );
  ...
  selectedRows: PropTypes.arrayOf(
    PropTypes.shape({
      original: PropTypes.shape({
        ...
        usageLocations: PropTypes.arrayOf(
          PropTypes.shape({

...
import { Card, Form, Col, Button } from '@openedx/paragon';
import { useIntl } from '@edx/frontend-platform/i18n';

import CourseUploadImage from '../../../generic/course-upload-image';
import messages from './messages';

const InstructorContainer = ({ instructor, idx, onDelete, onChange }) => {
  const intl = useIntl();
  return (
        ...
        <Form>
          <Form.Row>
            <Form.Group as={Col} className="form-group-custom">
              ...
              />
              <Form.Text>{intl.formatMessage(messages.instructorNameHelpText)}</Form.Text>
            </Form.Group>

            <Form.Group as={Col} className="form-group-custom">
              <Form.Label>{intl.formatMessage(messages.instructorTitleLabel)}</Form.Label>
              <Form.Control
                value={instructor?.title}
                placeholder={intl.formatMessage(messages.instructorTitleInputPlaceholder)}
                onChange={(e) => onChange(e.target.value, idx, 'title')}
              />
              <Form.Text>{intl.formatMessage(messages.instructorTitleHelpText)}</Form.Text>
            </Form.Group>

            <Form.Group as={Col} className="form-group-custom">
              <Form.Label>{intl.formatMessage(messages.instructorOrganizationLabel)}</Form.Label>
              <Form.Control
              ...
              />
              <Form.Text>{intl.formatMessage(messages.instructorOrganizationHelpText)}</Form.Text>
            </Form.Group>
          </Form.Row>
          <Form.Row>
            <Form.Group as={Col} className="form-group-custom">
              <Form.Label>{intl.formatMessage(messages.instructorBioLabel)}</Form.Label>
              <Form.Control
                as={TextareaAutosize}
              ...
              />
              <Form.Text>{intl.formatMessage(messages.instructorBioHelpText)}</Form.Text>
            </Form.Group>
          </Form.Row>
          <Form.Row className="pl-1 pr-2.5">
            <CourseUploadImage
              ...
              customInputPlaceholder={intl.formatMessage(messages.instructorPhotoInputPlaceholder)}

...
const UsageList = ({ className, itemList, isExperiment }) => {
  ...
  const usageDescription = isExperiment ? messages.experimentAccessTo : messages.accessTo;

  const renderValidationMessage = ({ text, type }) => (
    <span className="d-inline-flex align-items-center">
      <Icon src={MESSAGE_VALIDATION_TYPES.error === type ? ErrorIcon : WarningIcon} size="sm" className="mr-2" />
  ...
  return (
    <div className={className}>
      <p className="small text-gray-700 pt-2 pb-1.5 m-0">{formatMessage(usageDescription)}</p>
      <Stack gap={2}>
        {itemList.map(({ url, label, validation }) => (
          <>
            <Hyperlink className="small text-info-500" destination={formatUrlToUnitPage(url)} key={`${label} - ${url}`}>
              {label}
            </Hyperlink>

...
const renderComponent = (props = {}) =>
  render(
    <IntlProvider locale="en">
      <ContentGroupsSection availableGroup={contentGroupsMock} contentGroupActions={contentGroupActions} {...props} />
    </IntlProvider>,
  );
...
describe('<ContentGroupsSection />', () => {
  it('renders component correctly', () => {
    ...
    expect(getAllByTestId('content-group-card')).toHaveLength(contentGroupsMock.groups.length);
  });
  ...
  it('renders container with new group on create click if section is empty', async () => {
    ...
    await user.click(getByRole('button', { name: placeholderMessages.button.defaultMessage }));
  ...
  it('renders container with new group on create click if section has groups', async () => {
    ...
    await user.click(getByRole('button', { name: messages.addNewGroup.defaultMessage }));
    expect(getByTestId('content-group-form')).toBeInTheDocument();
  });
});

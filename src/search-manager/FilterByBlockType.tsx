...
import { Badge, Form, Icon, IconButton, Menu, MenuItem, ModalPopup, useToggle } from '@openedx/paragon';
...
const ProblemFilterItem = ({ count, handleCheckboxChange }: ProblemFilterItemProps) => {
  ...
  const { problemTypes, typesFilter, setTypesFilter } = useSearchContext();
  const intl = useIntl();
  ...
  const handleProblemCheckboxChange = React.useCallback(
    (e) => {
  ...
  return (
    <div className="problem-menu-item">
      ...
      <ModalPopup positionRef={problemItemTarget} isOpen={isProblemItemOpen} onClose={closeProblemItem}>
        <div className="bg-white rounded shadow problem-sub-menu-item">
          <Form.Group className="mb-0">
            <Form.CheckboxSet name="block-type-filter" value={[...typesFilter.problems]}>
              <Menu>
...
const FilterItem = ({ blockType, count }: FilterItemProps) => {
  const { problemTypes, setTypesFilter } = useSearchContext();
  ...
  if (blockType === 'problem') {
    ...
    return <ProblemFilterItem count={count} handleCheckboxChange={handleCheckboxChange} />;
  }

  return (
    <MenuItem as={Form.Checkbox} value={blockType} onChange={handleCheckboxChange}>
      <div>
        <BlockTypeLabel blockType={blockType} />{' '}
        <Badge variant="light" pill>
          {count}
...
const FilterByBlockType: React.FC<Record<never, never>> = () => {
  const { blockTypes, typesFilter, setTypesFilter } = useSearchContext();
  ...
  const appliedFilters = [...typesFilter.blocks, ...typesFilter.problems].map((blockType) => ({
  ...
  return (
    ...
    >
      <Form.Group className="mb-0">
        <Form.CheckboxSet name="block-type-filter" value={[...typesFilter.blocks]}>

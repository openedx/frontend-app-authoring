...
import { Chip, Button, Icon, useCheckboxSetValues, useToggle, StatefulButton } from '@openedx/paragon';
import { ArrowDropDown, CloseSmall, SpinnerSimple } from '@openedx/paragon/icons';
...
import { updateAllPreviousRunLinks, updateSinglePreviousRunLink, fetchRerunLinkUpdateStatus } from '../data/thunks';
...
const ScanResults: FC<Props> = ({
  ...
  const renderableSections = useMemo(() => {
    const buildSectionData = (items: any[], sectionId: string, messageKey: keyof typeof messages) => {
    ...
    if (data?.customPages && data.customPages.length > 0) {
      const customPagesSection = buildSectionData(data.customPages, 'custom-pages', 'customPagesHeader');
  ...
  const allSections = useMemo(() => [...renderableSections, ...(sections || [])], [renderableSections, sections]);

  const { brokenLinksCounts, lockedLinksCounts, externalForbiddenLinksCounts } = useMemo(
    () => countBrokenLinks({ sections: allSections }),
    [allSections],
  );
  ...
  const [filterBy, { add, remove, set, clear }] = useCheckboxSetValues(activeFilters);
  ...
  const processUpdateResults = useCallback(
    (response: any, isBulkUpdate = false) => {
      if (!response) {
        return;
      }
          ...
          section.subsections.forEach((sub) =>
            sub.units.forEach((unit) =>
              unit.blocks.forEach((b) => {
                if (b.previousRunLinks?.length) {
                  blocksWithResults.add(b.id);
                }
              }),
            ),
          );
          ...
          response.results.forEach((result) => {
            ...
            if (blockData) {
              ...
              if (result.success && newUrl && originalUrl) {
                const matchingLink = blockData.previousRunLinks.find(({ originalLink }) => {
  ...
  const getContentType = useCallback((sectionId: string): string => {
    if (sectionId === 'course-updates') {
      return 'course_updates';
    }
    if (sectionId === 'custom-pages') {
      return 'custom_pages';
    }
  ...
  const areAllLinksUpdated = useMemo(() => {
    ...
    if (allPreviousRunLinks.length === 0) {
      return false;
    }

    const allUpdated = allPreviousRunLinks.every(
      ({ linkId, isUpdatedInAPI }) => isUpdatedInAPI || updatedLinkIds.includes(linkId),
    );
  ...
  const handleUpdateLink = useCallback(
    async (link: string, blockId: string, sectionId?: string): Promise<boolean> => {
        ...
        const pollForSingleLinkResult = async (attempts = 0): Promise<boolean> => {
          ...
          if (updateStatusResponse && updateStatusResponse.results && updateStatusResponse.results.length > 0) {
            ...
            if (hasOriginalUrlField) {
              exactMatch = updateStatusResponse.results.find((result: any) => {
                const matches = result.id === blockId && result.original_url === link && result.success === true;
            ...
            const failed = updateStatusResponse.results.find((result: any) => {
              if (hasOriginalUrlField) {
                return result.id === blockId && result.original_url === link && result.success === false;
              }
    ...
    const hasVisibleUnit = section.subsections.some((subsection) =>
      subsection.units.some((unit) =>
        unit.blocks.some((block) => {
          ...
          return showBroken || showLocked || showExternal || (noFilters && (hasBroken || hasLocked || hasExternal));
        }),
      ),
    );
        ...
        {(() => {
          ...
          const visibleSections =
            allSections && allSections.length > 0
              ? allSections.map((_, index) => (shouldSectionRender(index) ? index : -1)).filter((idx) => idx !== -1)
              : [];
                  ...
                  <>
                    {subsection.units.map((unit) => {
                      ...
                      const hasVisibleBlock = unit.blocks.some((block) => {
                        ...
                        const noFilters =
                          !filters.brokenLinks && !filters.lockedLinks && !filters.externalForbiddenLinks;
